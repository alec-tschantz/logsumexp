...
\begin{equation}
    ...
    \section{Full Model: Spatiotemporal Slot Attention}

    \paragraph{Setup.}
    We have two sets of variables, data $\{\bm{x}_{p,t}\in\mathbb{R}^d : p=1,\ldots,N,\;\;t=1,\ldots,T\}$ and latents $\{\bm{s}_{k,t}\in\mathbb{R}^d : k=1,\ldots,K,\;\;t=1,\ldots,T\}$.
    Each time step \(t\) has \(N\) data vectors \(\bm{x}_{p,t}\) (e.g. pixels) and \(K\) latent vectors \(\bm{s}_{k,t}\).  We split the overall energy into three terms:
    
    For each \(t\) and each latent \(\bm{s}_{k,t}\), we define an energy term where \(\bm{s}_{k,t}\) is the \emph{parent} and the data \(\bm{x}_{p,t}\) (for \(p=1,\dots,N\)) are its \emph{parents}. Concretely,
    \[
    E^{\mathrm{data}}
    = 
    -\sum_{t=1}^T\sum_{k=1}^K
    \ln\Bigl(\sum_{p=1}^N
    \exp\!\bigl(\mathrm{sim}^{\mathrm{data}}(\bm{s}_{k,t},\bm{x}_{p,t})\bigr)\Bigr).
    \]
    
    For each \(t\), each latent \(\bm{s}_{k,t}\) is a \emph{child} whose \emph{parents} are the other latents \(\{\bm{s}_{k',t}\mid k'\neq k\}\) at the same time step:
    \[
    E^{\mathrm{concurrent}}
    =
    -\sum_{t=1}^T\sum_{k=1}^K
    \ln\Bigl(\sum_{\substack{k'=1 \\ k'\neq k}}^K
    \exp\!\bigl(\mathrm{sim}^{\mathrm{slot}}(\bm{s}_{k,t},\bm{s}_{k',t})\bigr)\Bigr).
    \]
    
    For each slot index \(k\), each \(\bm{s}_{k,t}\) is a \emph{child} whose \emph{parents} are its own past latents \(\{\bm{s}_{k,t'}\mid t'<t\}\).  A simple causal version blocks any \(t'\ge t\):
    \[
    E^{\mathrm{time}}
    =
    -\sum_{k=1}^K\sum_{t=2}^T
    \ln\Bigl(\sum_{t'<\,t}
    \exp\!\bigl(\mathrm{sim}^{\mathrm{time}}(\bm{s}_{k,t},\bm{s}_{k,t'})\bigr)\Bigr).
    \]
    ...
    \paragraph{Parameters and Similarity Functions.}
    We define distinct projection matrices for each type of interaction.  For latent--data,
    \[
    \bm{q}_{k,t}^{\mathrm{data}}=\bm{W}_{k}^{Q,\mathrm{data}}\bm{s}_{k,t}, 
    \quad
    \bm{k}_{p,t}^{\mathrm{data}}=\bm{W}^{K,\mathrm{data}}\bm{x}_{p,t},
    \quad
    \mathrm{sim}^{\mathrm{data}}(\bm{s}_{k,t},\bm{x}_{p,t})
    =
    (\bm{q}_{k,t}^{\mathrm{data}})^\top(\bm{k}_{p,t}^{\mathrm{data}}).
    \]
    For concurrency (latents within the same \(t\)),
    \[
    \bm{q}_{k,t}^{\mathrm{slot}}=\bm{W}_{k}^{Q,\mathrm{slot}}\bm{s}_{k,t}, 
    \quad
    \bm{k}_{k',t}^{\mathrm{slot}}=\bm{W}_{k'}^{K,\mathrm{slot}}\bm{s}_{k',t},
    \quad
    \mathrm{sim}^{\mathrm{slot}}(\bm{s}_{k,t},\bm{s}_{k',t})
    =
    (\bm{q}_{k,t}^{\mathrm{slot}})^\top(\bm{k}_{k',t}^{\mathrm{slot}}).
    \]
    For temporal attention of slot \(k\) across time,
    \[
    \bm{q}_{k,t}^{\mathrm{time}}=\bm{W}_{k}^{Q,\mathrm{time}}\bm{s}_{k,t}, 
    \quad
    \bm{k}_{k,t'}^{\mathrm{time}}=\bm{W}_{k}^{K,\mathrm{time}}\bm{s}_{k,t'},
    \quad
    \mathrm{sim}^{\mathrm{time}}(\bm{s}_{k,t},\bm{s}_{k,t'})
    =
    (\bm{q}_{k,t}^{\mathrm{time}})^\top(\bm{k}_{k,t'}^{\mathrm{time}}).
    \]
    All these matrices \(\bm{W}\) are learnable model parameters.
    
    \paragraph{Gradient Updates.}
    Each latent \(\bm{s}_{k,t}\) appears in \emph{three} child-energy terms (data, concurrency, time) and also as a \emph{parent} in the concurrency and time energies for \emph{others}:
    \[
    -\frac{\partial E^{\mathrm{Full}}}{\partial \bm{s}_{k,t}}
    =
    -\frac{\partial E^{\mathrm{data}}}{\partial \bm{s}_{k,t}}
    \;-\;
    \frac{\partial E^{\mathrm{concurrent}}}{\partial \bm{s}_{k,t}}
    \;-\;
    \frac{\partial E^{\mathrm{time}}}{\partial \bm{s}_{k,t}}.
    \]
    Below is a schematic decomposition (focusing on the child side plus parent side).  Let
    \[
        \alpha_{(k,t)\to (p,t)}^{\mathrm{data}}
        =
        \mathrm{softmax}_p\!\bigl(\mathrm{sim}^{\mathrm{data}}(\bm{s}_{k,t},\bm{x}_{p,t})\bigr),
        \]
        \[
        \alpha_{(k,t)\to (k',t)}^{\mathrm{slot}}
        =
        \mathrm{softmax}_{k'}\!\bigl(\mathrm{sim}^{\mathrm{slot}}(\bm{s}_{k,t},\bm{s}_{k',t})\bigr),
        \]
        \[
        \alpha_{(k,t)\to (k,t')}^{\mathrm{time}}
        =
        \mathrm{softmax}_{t'}\!\bigl(\mathrm{sim}^{\mathrm{time}}(\bm{s}_{k,t},\bm{s}_{k,t'})\bigr).
        \]
        
    Then, for the \emph{child side} of \(\bm{s}_{k,t}\) in each energy, we get a sum over its parents weighted by \(\alpha\).  In concurrency and time, \(\bm{s}_{k,t}\) can also act as a \emph{parent} for other latents \(\bm{s}_{k',t}\) or \(\bm{s}_{k,t'}\), yielding an additional sum over children \((k',t)\) or \((k,t')\) that attend \(\bm{s}_{k,t}\).  Combining these yields a final gradient of the form:
    \[
        -\frac{\partial E^{\mathrm{Full}}}{\partial \bm{s}_{k,t}}
        =
        \underbrace{
        \sum_{p=1}^N
        \alpha_{(k,t)\to (p,t)}^{\mathrm{data}}
        \;\bm{W}_{k}^{Q,\mathrm{data}\top}\,\bm{W}^{K,\mathrm{data}}\,\bm{x}_{p,t}
        }_{\text{(latents attending data)}}
        \]
        \[
        \quad+\;
        \underbrace{
        \sum_{\substack{k'=1 \\ k'\neq k}}^K
        \alpha_{(k,t)\to (k',t)}^{\mathrm{slot}}
        \;\bm{W}_{k}^{Q,\mathrm{slot}\top}\,\bm{W}_{k'}^{K,\mathrm{slot}}\,\bm{s}_{k',t}
        \;+\;
        \sum_{\substack{k'=1 \\ k'\neq k}}^K
        \alpha_{(k',t)\to (k,t)}^{\mathrm{slot}}
        \;\bm{W}_{k'}^{K,\mathrm{slot}\top}\,\bm{W}_{k}^{Q,\mathrm{slot}}\,\bm{s}_{k',t}
        }_{\text{(two-way concurrency: child side + parent side)}}
        \]
        \[
        \quad+\;
        \underbrace{
        \sum_{t'<t}
        \alpha_{(k,t)\to (k,t')}^{\mathrm{time}}
        \;\bm{W}_{k}^{Q,\mathrm{time}\top}\,\bm{W}_{k}^{K,\mathrm{time}}\,\bm{s}_{k,t'}
        \;+\;
        \sum_{t''>t}
        \alpha_{(k,t'')\to (k,t)}^{\mathrm{time}}
        \;\bm{W}_{k}^{K,\mathrm{time}\top}\,\bm{W}_{k}^{Q,\mathrm{time}}\,\bm{s}_{k,t''}
        }_{\text{(temporal: child side for $t$ and parent side for $\{t''>t\}$)}}.
        \]
        
    This final expression shows how \(\bm{s}_{k,t}\) collects gradients from (i) its data “parents,” (ii) concurrency interactions at the same time step, and (iii) temporal interactions with its own past or future (if allowed).  In a purely causal model, we can omit the \(\sum_{t''>t}\) term for parent-side time attention.
    
    \paragraph{Remark (Full Version).}
    We can generalize to let each latent \(\bm{s}_{k,t}\) attend \emph{all} data features and \emph{all} slot latents at \emph{any} time.  This merges the three energies into a single large log-sum-exp, imposing direct competition among data, concurrency, and temporal parents, but increases complexity to \(\mathcal{O}((N + K)\,T)^2\).  The above factorization typically suffices and keeps each family of interactions (latent--data, concurrency, time) more interpretable.

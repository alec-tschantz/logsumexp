...
% denote the \(k\)-th variable (or ``slot'') in layer \(l\) at time \(t\).  The lowest layer \(l=1\) has \(K_1 = N\) observed variables (e.g.\ pixels) and dimension \(D_1\) (e.g.\ \([x,y,r,g,b]\)), while higher layers have separate dimensions \(D_l\) and numbers of slots \(K_l\).  Our goal is to define an energy that couples these variables \emph{vertically} (across layers), \emph{concurrently} (within the same layer and time), and \emph{temporally} (across times). The number of variables at each layer generally decreases, \(N = K_1 > K_2 > \cdots > K_L\).

% \paragraph{Similarity Functions.}
% We introduce three types of similarity, each with its own projection matrices.  For vertical connections (e.g.\ linking layers \(l-1\) and \(l\)), we define
% \begin{equation}
% \mathrm{sim}_{v}^{(l)}(\bm{x}, \bm{y})
% \;=\;
% \bigl(\bm{W}_{v}^{Q,(l)}\,\bm{x}\bigr)^\top
% \bigl(\bm{W}_{v}^{K,(l)}\,\bm{y}\bigr),
% \end{equation}
% for concurrency (slots within the same layer and time),
% \begin{equation}
% \mathrm{sim}_{c}^{(l)}(\bm{x}, \bm{y})
% \;=\;
% \bigl(\bm{W}_{c}^{Q,(l)}\,\bm{x}\bigr)^\top
% \bigl(\bm{W}_{c}^{K,(l)}\,\bm{y}\bigr),
% \end{equation}
% and for temporal connections (the same slot across different times),
% \begin{equation}
% \mathrm{sim}_{t}^{(l)}(\bm{x}, \bm{y})
% \;=\;
% \bigl(\bm{W}_{t}^{Q,(l)}\,\bm{x}\bigr)^\top
% \bigl(\bm{W}_{t}^{K,(l)}\,\bm{y}\bigr).
% \end{equation}
% Here, each \(\bm{W}_{\bullet}^{Q,(l)}\) and \(\bm{W}_{\bullet}^{K,(l)}\) is a learnable parameter matrix for layer \(l\).

% \paragraph{Energy.}
% At each layer \(l\), we split the total energy into three log-sum-exp terms:
% \begin{equation}
% \begin{aligned}
% E_{v}^{(l)}
% &=\;
...
% \sum_{k=1}^{K_l}
% \exp \bigl(
% \mathrm{sim}_{v}^{(l)}(\bm{x}_{k,t}^{(l)}, \bm{x}_{c,t}^{(l-1)})
% \bigr)
% \Bigr),
% \\
% E_{c}^{(l)}
...
% \sum_{t'<t}
% \exp \bigl(
% \mathrm{sim}_{t}^{(l)}(\bm{x}_{k,t}^{(l)}, \bm{x}_{k,t'}^{(l)})
% \bigr)
% \Bigr).
% \end{aligned}
% \end{equation}
% Summing over all layers \(l\in\{1,\dots,L\}\) yields the total energy \(E\).  These three terms mirror familiar patterns in slot-attention (\(E_{v}^{(l)}\)), self-attention within a layer (\(E_{c}^{(l)}\)), and causal self-attention across time (\(E_{t}^{(l)}\)).

% \paragraph{Four Partial Derivatives.}
% Now, consider one variable \(\bm{x}_{k,t}^{(l)}\).  Its gradient factors into four components:
% \begin{equation}
%     \begin{aligned}
%     -\frac{\partial E}{\partial \bm{x}_{k,t}^{(l)}}
%     &= \underbrace{\text{bottom-up from explaining children in layer }(l-1)}_{\text{vertical}} \\
%     &\quad + \underbrace{\text{top-down from parents in layer }(l+1)}_{\text{vertical}} \\
%     &\quad + \underbrace{\text{lateral with other variables at time }t}_{\text{concurrency}} \\
%     &\quad + \underbrace{\text{temporal from variables with past times }t'<t}_{\text{temporal}}.
%     \end{aligned}
%     \end{equation}


% \paragraph{Attention Weights.}
% Each log-sum-exp implicitly defines a \(\mathrm{softmax}\) over possible ``parents.''  For example, in the vertical energy \(E_{v}^{(l)}\), each child \(\bm{x}_{c,t}^{(l-1)}\) competes among the possible parents \(\{\bm{x}_{k,t}^{(l)}\}\).  We denote the resulting attention weight by
% \begin{equation}
% \alpha_{v}^{(l)}(c\!\to\!k)
% \;=\;
% \frac{\exp\bigl(\mathrm{sim}_{v}^{(l)}(\bm{x}_{k,t}^{(l)},\bm{x}_{c,t}^{(l-1)} )\bigr)}
% {\sum_{k''=1}^{K_l}
%  \exp\bigl(\mathrm{sim}_{v}^{(l)}(\bm{x}_{k'',t}^{(l)}, \bm{x}_{c,t}^{(l-1)})\bigr)}.
% \end{equation}
% Similarly, \(\alpha_{c}^{(l)}(k\!\to\!k')\) emerges from \(E_{c}^{(l)}\) (slots in the same layer and time), and \(\alpha_{t}^{(l)}(t\!\to\!t')\) emerges from \(E_{t}^{(l)}\) (the same slot across time).  These attention weights directly parallel the ``responsibilities'' in GMM or the attention probabilities in Transformers.

% \paragraph{Bottom-up Gradient:}
% \begin{equation}
...
% \mathrm{sim}_{v}^{(l)}\bigl(\bm{x}_{k,t}^{(l)},\bm{x}_{c,t}^{(l-1)}\bigr).
% \end{equation}
% This term mirrors Slot-Attention or Gaussian-mixture updates, where each parent \(\bm{x}_{k,t}^{(l)}\) aggregates contributions from the children \(\bm{x}_{c,t}^{(l-1)}\), weighted by \(\alpha_{v}^{(l)}\).

% \paragraph{Top-down Gradient:}
% \begin{equation}
...
% \alpha_{v}^{(l+1)}(k\!\to\!p)
% \;\frac{\partial}{\partial \bm{x}_{k,t}^{(l)}}
% \mathrm{sim}_{v}^{(l+1)}\bigl(\bm{x}_{p,t}^{(l+1)}, \bm{x}_{k,t}^{(l)}\bigr).
% \end{equation}
% Here, \(\bm{x}_{k,t}^{(l)}\) appears as a child for the layer \(l+1\).  This mirrors the parent-term in our earlier Hopfield/Self-Attention derivations.

% \subparagraph{Inter-layer Gradient}
% In standard self-attention, each token both attends to others (child perspective) and is attended by others (parent perspective).  Hence we split it across two lines:
% \begin{equation}
% \begin{aligned}
...
% \alpha_{c}^{(l)}(k\!\to\!k')
% \;\frac{\partial}{\partial \bm{x}_{k,t}^{(l)}}
% \mathrm{sim}_{c}^{(l)}\bigl(\bm{x}_{k,t}^{(l)}, \bm{x}_{k',t}^{(l)}\bigr)
% }_{\text{child perspective}}\\
% &\quad+\;
% \underbrace{\sum_{k'\neq k}
% \alpha_{c}^{(l)}(k'\!\to\!k)
% \;\frac{\partial}{\partial \bm{x}_{k,t}^{(l)}}
% \mathrm{sim}_{c}^{(l)}\bigl(\bm{x}_{k',t}^{(l)}, \bm{x}_{k,t}^{(l)}\bigr)
% }_{\text{parent perspective}}.
% \end{aligned}
% \end{equation}
% This exactly generalizes our Hopfield or self-attention updates in the earlier document.

% \subparagraph{Temporal Gradient}
% Likewise, each \(\bm{x}_{k,t}^{(l)}\) attends to its own past \(\{\bm{x}_{k,t'}^{(l)} : t'<t\}\) and is attended by its future \(\{\bm{x}_{k,t''}^{(l)} : t''>t\}\) if non-causal attention is allowed.  We again split child vs.\ parent:
% \begin{equation}
% \begin{aligned}
...
% \;\frac{\partial}{\partial \bm{x}_{k,t}^{(l)}}
% \mathrm{sim}_{t}^{(l)}\bigl(\bm{x}_{k,t''}^{(l)}, \bm{x}_{k,t}^{(l)}\bigr)
% }_{\text{parent perspective (if non-causal)}}.
% \end{aligned}
% \end{equation}
% This parallels Transformer-style causal masking across time, matching the earlier self-attention across sequences.
